...
3. When prompted, enter the thermostat profile name exposed by the host-side thermostat proxy.
...
| ------------------ | -------- | ------------------------------------------------------------------------ |
| Thermostat Proxy Profile | Yes      | Profile name resolved by the host-side thermostat proxy. Default: `primary`. |
...
The host-side thermostat proxy must provide:

- a Unix socket mounted in Home Assistant at `/run/adc-thermostat-proxy/proxy.sock`
